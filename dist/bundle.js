...
riot$1.tag2('color-tip', '<div class="tip" title="{title}" riot-style="background-color: {color};"></div>', 'color-tip .tip,[data-is="color-tip"] .tip{ width: 20px; height: 20px; margin:0; padding:0; display:inline-block; }', 'class="tip"', function(opts) {
    this.title = this.name + ' : ' + this.color;
});

riot$1.tag2('color-lists', '<div class="wrapper"> <select name="colorlists" id="colorlists" onchange="{colorListLord}"> <option each="{colorlistData}" riot-value="{name}">{name}</option> </select> <div class="scrollbar-wrapper"> <div class="scrollbar-body"> <div ref="colortips" class="scrollbar-content"> <color-tip each="{colorlists}"></color-tip> </div> </div> </div> </div>', 'color-lists #colorlists,[data-is="color-lists"] #colorlists{ width: 280px; font-size: 20px; margin: 10px 0; display: block; } color-lists #colorlists option,[data-is="color-lists"] #colorlists option{ background: #fff; color: #111; } color-lists #colorlists option:hover,[data-is="color-lists"] #colorlists option:hover{ background: aquamarine; } color-lists .wrapper,[data-is="color-lists"] .wrapper{ position: relative; height: calc(100% - 420px); margin: 0; } color-lists .scrollbar-wrapper,[data-is="color-lists"] .scrollbar-wrapper{ position: relative; height: 100%; overflow: hidden; } color-lists .scrollbar-body,[data-is="color-lists"] .scrollbar-body{ width: calc(100% + 17px); height: 100%; overflow-y: scroll; } color-lists .scrollbar-content,[data-is="color-lists"] .scrollbar-content{ display: flex; flex-wrap: wrap; }', '', function(opts) {
    ...
    this.colorlistData = [
      { name: 'Web Color',
        list: parser( WEBCOLOR, null, true) },
      { name: 'JIS EN',
...
riot$1.tag2('context-menu', '<div id="menu" ref="menu" show="{mode}"> <p class="menuitem" onclick="{addCard}" if="{mode == \'tip\'}">ADD CARD</p> <p class="menuitem" onclick="{removeCard}" if="{mode == \'card\'}">DELETE</p> <p class="menuitem" onclick="{duplicateCard}" if="{mode == \'card\'}">DUPLICATE</p> <p class="menuitem" onclick="{modeChange}" if="{mode == \'card\'}">Mode</p> <p class="menuitem" onclick="{setBgColor}">SET BACKGROUND</p> <p class="menuitem"> <span>COPY:</span> <span each="{key in copys}" class="menuitem" onclick="{copyColor}">{key}</span> </p> <p class="menuitem" if="{mode == \'card\'}"> <span>SIZE:</span> <span each="{key in sizes}" class="menuitem" onclick="{setSize}">{key}</span> </p> </div>', 'context-menu #menu,[data-is="context-menu"] #menu{ position: absolute; font-size:12px; background: #fff; border: solid 1px silver; z-index: 100; } context-menu .menuitem,[data-is="context-menu"] .menuitem{ min-width: 100px; padding: 4px; margin: 0; } context-menu .menuitem:hover,[data-is="context-menu"] .menuitem:hover,context-menu .menuitem:active,[data-is="context-menu"] .menuitem:active{ background: aquamarine; } context-menu .menuitem .menuitem:hover,[data-is="context-menu"] .menuitem .menuitem:hover{ font-weight: bold; }', '', function(opts) {
